...
import { removeANSI, sortBasedOnLength } from "../libs/stringUtils";
 ...
 */
const checkIfTable: (row: string | undefined, verticalLine: string) => boolean =
    (row, verticalLine) => {
...
export const generateTable: (
    ...
    rows.forEach((row, index) => {
        ...
        if (!row.includes("┤")) finalRows.push(separator);
    });
...
export const generateColorTable: (
    ...
    conversionKeys = conversionKeys.sort(sortBasedOnLength);
